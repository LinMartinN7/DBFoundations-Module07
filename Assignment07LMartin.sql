...
-- 2021-05-26, Lin Martin,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_LinMartin')
	 Begin 
	  Alter Database [Assignment07DB_LinMartin] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_LinMartin;
	 End
	Create Database Assignment07DB_LinMartin;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_LinMartin;
...
SELECT ProductName, InventoryDate = DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate), [Count]
...
CREATE or ALTER VIEW vProductsInventories
AS
SELECT TOP 1000000 ProductName, InventoryDate = DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate), [Count]
FROM vProducts Inner Join vInventories
ON vProducts.ProductID = vInventories.ProductID
ORDER BY ProductName, InventoryDate, [Count];
GO
...
AS
SELECT TOP 1000000 CategoryName, InventoryDate = DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate),
...
AS
SELECT TOP 1000000 ProductName, InventoryDate = DateName(Month, InventoryDate) + ', ' + DateName(Year, InventoryDate),
	InventoryCount = [Count], PreviousMonthCount = IsNull(Lag(Sum(Count)) Over(PARTITION BY ProductName ORDER BY Month(InventoryDate)),0)
...
CREATE or ALTER VIEW vProductInventoriesWithPreviouMonthCountsWithKPIs
AS
SELECT TOP 1000000 ProductName, InventoryDate, InventoryCount, CountVsPreviousCountKPI =
	CASE
...
AS
RETURN
	SELECT TOP 1000000 * FROM vProductInventoriesWithPreviouMonthCountsWithKPIs
